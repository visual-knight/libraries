...
async function normalizeRetinaScreenshot(
  ...
  const viewportSizeMax = screenDimensions.applyScaleFactor(
    Math.max(screenDimensions.getViewportWidth(), screenDimensions.getViewportHeight()),
  );
  const viewportSizeMin = screenDimensions.applyScaleFactor(
    Math.min(screenDimensions.getViewportWidth(), screenDimensions.getViewportHeight()),
  );
  const isImageScaled = imageSizeMax !== viewportSizeMax && imageSizeMin !== viewportSizeMin;

  if (isImageScaled) {
    const normalizedScreenshot = await scaleImage(base64Screenshot, 1 / screenDimensions.getPixelRatio()) as Base64;
    return normalizedScreenshot;
  }
  return base64Screenshot;
}

async function normalizeIOSScreenshot(browser: any, screenDimensions: ScreenDimensions, base64Screenshot: Base64) {
...
export default async function normalizeSreenshot(
  browser: any,
  screenDimensions: ScreenDimensions,
  base64Screenshot: Base64,
) {
